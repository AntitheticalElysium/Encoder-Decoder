...
def download_and_save_raw_data(root="data", filename="raw.txt"):
    ...
    with (
        open(fra_txt_path, "r", encoding="utf-8") as f_in,
...
def normalize_corpus(raw_path="data/raw.txt", norm_path="data/norm.txt"):
    with (
        open(raw_path, "r", encoding="utf-8") as f_in,
...
def build_vocabulary(filtered_pairs, min_freq=1):
    eng_sentences, fra_sentences = zip(*filtered_pairs)
    vocab_src = Vocab()  # English
    vocab_tgt = Vocab()  # French

    # Train BPE with reasonable merge counts for dataset size
    # Reduced from 8000 to 1000 for better performance and quality
    vocab_src.train_bpe(eng_sentences, num_merges=1000)
    vocab_tgt.train_bpe(fra_sentences, num_merges=1000)
...
def save_preprocessed_data(
    ...
    vocab_src,
    vocab_tgt,
    save_path="data/preprocessed_data.pkl",
):
    data = {
...
if __name__ == "__main__":
    ...
    src_ids_list, tgt_input_ids_list, tgt_output_ids_list = convert_to_ids(
        filtered_pairs, vocab_src, vocab_tgt
    )
    save_preprocessed_data(
        src_ids_list, tgt_input_ids_list, tgt_output_ids_list, vocab_src, vocab_tgt
    )
